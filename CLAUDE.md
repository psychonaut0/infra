...
- **Desktop:** Hyprland under SDDM (astronaut `blvck` theme). Power profiles come from **asusd**, not power-profiles-daemon — PPD is masked *and uninstalled* because both drive `platform_profile` and race. `asusctl armoury` exposes the PPT limits (`ppt_pl1_spl` 28–80W, default 60) but **no iGPU memory allocation** — that stays a UEFI/Armoury Crate setting.
